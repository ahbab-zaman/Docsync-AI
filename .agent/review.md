...
* The project docs that define the system:
...
For this project, always keep Phase 1 scope in mind unless the developer explicitly says the work has moved beyond it.
...
* The project phase — is this still Phase 1 work, or did it drift into later phases?
...
* **Architecture boundaries** — does code in the right place own the right responsibilities?

  * No UI logic in backend-only modules
  * No data access in presentational components
  * No cross-layer shortcuts that break the project structure
  * `/src/app` should remain the app-router entry point
* **Design system** — are the correct tokens, classes, and patterns used?

  * Use `ui-tokens.md` values
  * Match `ui-rules.md`
  * Keep `ui-registry.md` patterns consistent
  * Flag any raw color classes, hardcoded values, or inconsistent radius/spacing usage
...
* **Library usage** — if the feature uses a library from `library-docs.md`, is it being used the project’s way?
...
* Data consistency — are UI, API, and state in sync?
* Realtime behavior — if the feature touches collaboration, are presence, sync, or event flows stable?
* AI behavior — if the feature touches AI, are prompts, outputs, and fallback states handled cleanly?
...
If an issue is minor, important, or critical, label it clearly so the developer can triage quickly.
...
* UI or token usage that conflicts with the project’s design system baseline
* Realtime or AI flows that break the project’s core behavior
...
* Library usage that differs from the project’s documented pattern

**Minor — fix when convenient**

* Naming inconsistencies that do not affect behavior
* Missing optimizations
* Style issues that do not affect the design system
* Small UX polish items that do not block use
...
* `/src/app` structure is preserved
* UI matches the design tokens and registry
* backend, realtime, and AI boundaries stay clean
* the feature matches the documented plan
* the implementation is ready for the next step without rework
