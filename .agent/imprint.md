...
name: imprint
description: After building any UI component, extract the visual patterns that matter for consistency and save them to ui-registry.md. Use the project’s design tokens and existing component patterns so every future component matches what came before.
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
If no filepath was provided — identify which component files were most recently created or modified in this session. Look in `src/components/`, `src/app/`, and any other locations where UI files typically live. Read those files.
...
* Any use of project design tokens from `ui-tokens.md`
...
* Prefer project tokens from `ui-tokens.md` over raw color values
* If a hardcoded color appears, note it as a problem instead of normalizing it silently
* If the component uses a token class like `bg-surface`, `border-border`, `text-text-primary`, capture that exact token class
* If the component has multiple visual variants, capture the base or dominant pattern first
* If the component is a reusable UI primitive, note whether it should match all future instances of the same primitive
...
At the start of any session that involves UI work, read `ui-registry.md` before writing any component. When building a new card, check how existing cards were built. When building a new button, check what button patterns already exist. When building a new status badge, match the exact classes already in use.

For this project, the registry should always reflect the actual design system in use:

* the **background and surface tokens**
* the **text hierarchy**
* the **accent color usage**
* the **spacing and radius conventions**
* the **shared button, card, input, badge, and panel patterns**
...
For this project, focus especially on:

* `src/components/`
* `src/app/`
* any reusable UI primitives
* any shared layout components
* any editor, sidebar, toolbar, badge, card, modal, or button files
