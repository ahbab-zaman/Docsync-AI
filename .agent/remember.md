...
## Project Context This Skill Should Preserve

This project is a **single-repo AI real-time collaboration web app**.

The important permanent context includes:

- Phase 1 only unless explicitly expanded
- project starts from an empty repo
- `/src` folder structure is required from the start
- Next.js + React + TypeScript are the frontend foundation
- realtime collaboration uses Tiptap + Yjs/Hocuspocus + Socket.IO
- backend uses Node.js + PostgreSQL + Prisma + Redis + BullMQ
- UI is premium, dark-first, and token-driven
...
**What was built** — specific files created or modified, features completed, components added. Be precise.

**Decisions made** — choices that would be hard to reverse or that future work depends on. Architectural choices matter more than implementation detail.

**Problems solved** — any issue that took time to figure out so the next session does not solve the same problem twice.
...
Do not start building. Do not assume the developer wants to continue immediately. Summarise what was restored so the developer can verify the context was understood correctly.
